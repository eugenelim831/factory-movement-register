...
function receiptItemsHtml(attempt, record) {
  ...
  return (attempt.items || []).map((check, index) => {
    ...
    return `<div class="received-item-line${issues.length ? " has-discrepancy" : ""}"><strong>Item ${index + 1}: ${escapeHtml(itemDisplayName(item))}</strong><span>Received: ${escapeHtml(check.quantityReceived ?? 0)} ${escapeHtml(item.unit || "")}</span>${issues.length ? `<span><strong>Discrepancies:</strong> ${issues.map(escapeHtml).join(", ")}</span>` : `<span>Discrepancies: None</span>`}</div>`;
...
function openRecordView(record) {
  ...
  $("#recordView").innerHTML = `
    ...
    ${viewerQr}
    <section class="record-section"><h3>Receiving History (${attempts.length})</h3>${attempts.length ? `<p class="empty-note">The latest attempt is open. Select any earlier attempt to view its details.</p><div class="receiving-history-list">${attempts.slice().reverse().map((attempt, reverseIndex) => { const attemptNumber = attempts.length - reverseIndex; const issues = (attempt.items || []).flatMap(item => item.discrepancies?.length ? item.discrepancies : item.discrepancyReason ? [item.discrepancyReason] : []); const resultText = escapeHtml((attempt.result || "—").replaceAll("_", " ")); return `<details class="receiving-attempt${issues.length ? " has-discrepancy" : ""}" ${reverseIndex === 0 ? "open" : ""}><summary><span>Attempt ${attemptNumber} · ${formatDate(attempt.checkedAt)}&nbsp;&nbsp;·&nbsp;&nbsp;${resultText}${issues.length ? " · DISCREPANCY" : ""}</span></summary><div class="receiving-attempt-body"><div class="record-table-wrap"><table class="details-table"><tbody><tr><th>Receiving PIC</th><td>${escapeHtml(attempt.receivedBy || "—")}</td><th>Entered By</th><td>${escapeHtml(attempt.checkedBy || "—")}</td></tr><tr><th>Condition</th><td>${escapeHtml(attempt.condition || "—")}</td><th>Remarks</th><td>${escapeHtml(attempt.remarks || "—")}</td></tr><tr><th>Items Received</th><td colspan="3">${receiptItemsHtml(attempt, record)}</td></tr><tr class="${issues.length ? "discrepancy-row" : ""}"><th>All Discrepancies</th><td colspan="3">${issues.length ? `<ul class="discrepancy-list">${issues.map(issue => `<li>${escapeHtml(issue)}</li>`).join("")}</ul>` : "None"}</td></tr></tbody></table></div></div></details>`; }).join("")}</div>` : `<p class="empty-note">No receiving attempt has been recorded.</p>`}</section>
